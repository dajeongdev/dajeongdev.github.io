...
---
현재 진행하는 프로젝트에서 쿠폰 관련 복잡한 쿼리를 작성하던 중이었다. SQL로 먼저 작업을 해보는데 서브쿼리를 from 절에 넣어야 데이터가 제대로 나오는데, QueryDSL에서는 from 절에 서브쿼리를 넣을 수 없었다. 그래서 GPT에게 문의하여 방법을 찾아보니 `@Subselect`를 사용하면 쿼리를 캡슐화해서 View를 만들어 엔티티처럼 사용하는 방법을 알게 되었다.
...
### 장점
- 이렇게 쿠폰처럼 중복으로 여러 곳에서 사용할 경우 하나의 클래스로 두는 것이 가독성이나 유지보수면에서 좋다.
